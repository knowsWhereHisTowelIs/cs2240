...
void execute(char *rawInputORIGANALLLLL) {
    char *processOutput = "asfd\0"; //initialize to null string

    // char *rawInput = "ls | grep .c";
    char *rawInput = "ls -l | ./Upcase | ./LowerHalf ";
    char **argp;
...
void handleSingleProcess(char *input, char *previousOutput, char **output) {
    ...
    *output = buffer;
    // memcpy(&output, &buffer[0], sizeof(buffer));
}
